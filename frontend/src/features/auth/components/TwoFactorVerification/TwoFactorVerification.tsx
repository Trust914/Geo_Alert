...
export function TwoFactorVerification() {
  ...
  const verifyMutation = useMutation({
    mutationFn: ({ code, token }: { code: string; token?: string }) =>
      authService.verify2FA({ code }, token),
    onSuccess: (data) => {
      setIsSuccess(true)
  ...
  const resendMutation = useMutation({
    mutationFn: authService.resend2FA,
    onSuccess: () => {
      setCountdown(60)
    ...
    return (
      <div className="flex flex-col items-center justify-center py-8 space-y-4">
        ...
        <div className="text-center space-y-1">
          <p className="text-lg font-semibold text-foreground">Verified Successfully</p>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col items-center space-y-2">
        ...
        <div className="text-center space-y-1">
          <p className="text-sm font-medium text-foreground">{getMethodLabel()} Verification</p>
      ...
      {/* Resend Code - only for EMAIL and SMS methods */}
      {state.method === "EMAIL" && (
